...
public class AnimatedBackground extends Background {
	...
	public AnimatedBackground(int level, String frameFolder, String fileEnding, int numFrames) {
		super(1.0f, 1.0f, 1.0f, level);
		this.numFrames = numFrames;
		textures = new int[numFrames];
		for(int i =1; i<= numFrames; i++) {
			String filePath = frameFolder + "\\frame" + String.format("%04d", i) + fileEnding;
			
			
			//
			Image image = Util.readImage(filePath, false);

			// bind to texture unit 0, since that is the unit we operate on
			GL13.glActiveTexture(GL13.GL_TEXTURE0);
			// generate the texture id
			textures[i-1] = GL11.glGenTextures();

			// bind to the generated texture object
			GL11.glBindTexture(GL30.GL_TEXTURE_2D, textures[i-1]);
	...
	public void render() {
		...
		GL20.glBindTexture(GL20.GL_TEXTURE_2D, textures[frame]);
