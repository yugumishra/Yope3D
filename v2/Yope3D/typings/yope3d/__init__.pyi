...
@overload
def reg_get(e: Entity, name: Literal["Transient"]) -> bool | None: ...
@overload
...
@overload
def reg_add(entity: Entity, name: Literal["Transform"]) -> Transform: ...
@overload
def reg_add(entity: Entity, name: Literal["Hull"]) -> Hull: ...
@overload
def reg_add(entity: Entity, name: Literal["SphereForm"]) -> SphereForm: ...
@overload
def reg_add(entity: Entity, name: Literal["AABBForm"]) -> AABBForm: ...
@overload
def reg_add(entity: Entity, name: Literal["OBBForm"]) -> OBBForm: ...
@overload
def reg_add(entity: Entity, name: Literal["CapsuleForm"]) -> CapsuleForm: ...
@overload
def reg_add(entity: Entity, name: Literal["CylinderForm"]) -> CylinderForm: ...
@overload
def reg_add(entity: Entity, name: Literal["CompoundCollider"]) -> CompoundCollider: ...
@overload
def reg_add(entity: Entity, name: Literal["Material"]) -> Material: ...
@overload
def reg_add(entity: Entity, name: Literal["LightSource"]) -> LightSource: ...
@overload
def reg_add(entity: Entity, name: Literal["Name"]) -> Name: ...
@overload
def reg_add(entity: Entity, name: Literal["SpringConstraint"]) -> SpringConstraint: ...
@overload
def reg_add(entity: Entity, name: Literal["PointJointConstraint"]) -> PointJointConstraint: ...
@overload
def reg_add(entity: Entity, name: Literal["HingeJointConstraint"]) -> HingeJointConstraint: ...
@overload
def reg_add(entity: Entity, name: Literal["ConeTwistJointConstraint"]) -> ConeTwistJointConstraint: ...
@overload
def reg_add(entity: Entity, name: Literal["Parent"]) -> Parent: ...
@overload
def reg_add(entity: Entity, name: Literal["ScriptComponent"]) -> ScriptComponent: ...
@overload
def reg_add(entity: Entity, name: Literal["UITransform"]) -> UITransform: ...
@overload
def reg_add(entity: Entity, name: Literal["UIBackground"]) -> UIBackground: ...
@overload
def reg_add(entity: Entity, name: Literal["UITexturedBackground"]) -> UITexturedBackground: ...
@overload
def reg_add(entity: Entity, name: Literal["UIText"]) -> UIText: ...
@overload
def reg_add(entity: Entity, name: Literal["UIButton"]) -> UIButton: ...
@overload
def reg_add(entity: Entity, name: Literal["TextLabel3D"]) -> TextLabel3D: ...
@overload
def reg_add(entity: Entity, name: Literal["AnimationPlayer"]) -> AnimationPlayer: ...
@overload
def reg_add(entity: Entity, name: Literal["AudioSource"]) -> AudioSource: ...
@overload
def reg_add(entity: Entity, name: Literal["Fixed"]) -> bool: ...
@overload
def reg_add(entity: Entity, name: Literal["Transient"]) -> bool: ...
@overload
def reg_add(entity: Entity, name: str) -> Any: ...
def reg_add(entity: Entity, name: str) -> Any:
    """Add a default-constructed component to an entity and return it.

    The returned reference is the component just added, so there is no need to
    ``reg_get`` it back::

        m = yope3d.reg_add(e, "Material")
        m.roughness = 0.85
    ...
    Returns:
        A **live reference** into engine memory (edits apply immediately). Unlike
        ``reg_get`` this is never ``None`` — an unknown name raises instead. The
        zero-size tags (``"Fixed"``, ``"Transient"``) carry no data and return
        ``True``.
    ...
    Warning:
        Composition change — invalidates references held to this entity's *other*
        components, and any earlier reference to this one (Hazard #1). The
        returned reference is taken after the migration, so it is valid until the
        next composition change.
    """
