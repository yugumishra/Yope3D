...
StorageBuffer::StorageBuffer(GpuDevice& gpu, VkDeviceSize size) : bufSize(size) {
...
void StorageBuffer::destroy(VkDevice device) {
...
void StorageBuffer::write(const void* data, VkDeviceSize size) {
    memcpy(mapped, data, static_cast<size_t>(size));
}

uint32_t StorageBuffer::findMemoryType(VkPhysicalDevice pd, uint32_t filter,
