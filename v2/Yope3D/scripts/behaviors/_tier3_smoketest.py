...
class Tier3Smoketest:
    ...
    def _drop(self):
        p = self.pos
        proj = yope.world.add_sphere(1.0, 0.4, yope.Vec3(p.x, p.y + 8.0, p.z))
        yope.world.attach_sphere_mesh(proj, 0.4, 0.3, 0.5, 1.0)
        yope.reg_get(proj, "Hull").velocity = yope.Vec3(0, -6, 0)  # nudge down
    ...
    def on_collision_enter(self, world, entity, other):
        ...
        print(f"[tier3] ENTER hit#{self.hits} {entity} <- {other} "
              f"behavior(other)={yope.get_behavior(other)}")
    ...
    def update(self, world, entity, dt):
        _timers.tick(dt)
        if yope.input.is_key_pressed(yope.KEY_R):
            self._drop()
