...
Attach to any entity with a Transform, press Play, and watch the console + viewport:
  ...
  - plays a one-shot sound via yope3d.play_sound
...
class Tier1Smoketest:
    ...
    def init(self, world, entity, params):
        ...
        cap = world.add_capsule(0.4, 0.6, 1.0, yope3d.Vec3(-2, 4, 0))
        world.attach_capsule_mesh(cap, 0.4, 0.6, 0.3, 0.7, 1.0)

        cyl = world.add_cylinder(0.5, 0.7, 1.0, yope3d.Vec3(2, 4, 0))
        ...
        ball = world.add_sphere(1.0, 0.5, yope3d.Vec3(0, 6, 0))
        world.attach_sphere_mesh(ball, 0.5, 0.9, 0.9, 0.3)

        world.add_point_light(yope3d.Vec3(0, 6, 3), yope3d.Vec3(1, 1, 1), 2.0)
        ...
        world.add_text_label_3d(font, "tier1!", yope3d.Vec3(0, 5, 0))

        snd = yope3d.play_sound(params.get("sound", "audios/test.ogg"))
    ...
    def update(self, world, entity, dt):
        ...
        if self.t >= 1.0:
            self.t = 0.0
            self.score += 1
            yope3d.set_text(self.score_ui, f"Score: {self.score}")

        if yope3d.input.is_mouse_pressed(yope3d.MOUSE_LEFT):
            print("[tier1] LMB click edge")

        sy = yope3d.input.get_scroll_y()
        if sy != 0.0:
