...
(it drives yope.camera) + gameplay behaviors on other entities. One ScriptComponent
...
def _draw_marker(pt, color, size=0.4):
    ...
    yope.draw_line(pt - yope.Vec3(size, 0, 0), pt + yope.Vec3(size, 0, 0), color)
    yope.draw_line(pt - yope.Vec3(0, size, 0), pt + yope.Vec3(0, size, 0), color)
    yope.draw_line(pt - yope.Vec3(0, 0, size), pt + yope.Vec3(0, 0, size), color)
...
class Tier2Smoketest:
    ...
    def init(self, world, entity, params):
        ...
        for i in range(5):
            ...
            yope.reg_get(e, "Name").value = f"target_{i}"
            yope.reg_get(e, "Hull").gravity = False   # hover until punched
            self.targets.append(e)

        # Quat math + lookup sanity (these ran fine for you already).
        q = yope.Quat.from_euler(0.0, yope.to_radians(90.0), 0.0)
        print(f"[tier2] init t={yope.time():.2f} aim={d} "
              f"rotated={q.rotate(yope.Vec3(0,0,1))} "
              f"found={yope.find_entity('target_0')} "
              f"look={yope.look_at(yope.Vec3(1,0,0))}")
    ...
    def update(self, world, entity, dt):
        ...
        hit, e, point, normal, t = yope.raycast(o, d, 100.0, entity)
        if hit:
            _draw_marker(point, yope.Vec3(0.2, 0.9, 1.0))

        if yope.input.is_mouse_pressed(yope.MOUSE_LEFT) and hit:
            world.set_mesh_color(e, 0.2, 1.0, 0.2)
            world.apply_impulse(e, d * 8.0)            # also wakes the body
            _draw_marker(point, yope.Vec3(1, 1, 0), 0.8)
