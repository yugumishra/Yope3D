...
def build_pyramid(world, base_n):
    ...
    world.add_static_aabb(yope3d.Vec3(0, -0.5, 0), yope3d.Vec3(half_floor, 0.5, 15))
    ...
    for row in range(base_n):
        ...
        for j in range(count):
            ...
            e = world.add_obb(yope3d.Vec3(PYR_HALF, PYR_HALF, PYR_HALF), 1.0, yope3d.Vec3(x, y, 0))
            r = 0.2 + t * 0.7
            g = 0.45 - t * 0.15
            b = 0.9  - t * 0.7
            world.attach_box_mesh(e, yope3d.Vec3(PYR_HALF, PYR_HALF, PYR_HALF), r, g, b)

    cam_z = base_n + 4.0
    cam_y = base_n * 0.7
    yope3d.camera.set_position(yope3d.Vec3(0, cam_y, cam_z))
    yope3d.camera.set_rotation(yope3d.Vec3(0, 0, 0))

# Run immediately when the script is executed in the Scene Script panel
build_pyramid(yope3d.world, 7)
